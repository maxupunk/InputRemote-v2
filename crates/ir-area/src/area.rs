//! O canal 4 visto de uma ponta: o texto oferecido, pedido, em pedaços e conferido.
//!
//! ```text
...
//! cai por uma mensagem que chegou (`Sender::within_ack_reach`, em `ir-confiabilidade`); com
//! a janela cheia, cai também (o envio da sessão). Sobre stream, o rádio é o mesmo que carrega o
...
use ir_proto::message::{ClipId, ClipKind, ClipboardMessage, DeclineReason};
...
pub const PEDACO: usize = MAX_RFCOMM_PLAINTEXT - 64;
...
impl Area {
    ...
    pub fn oferecer(&mut self, texto: ClipText) -> ClipboardMessage {
    ...
    pub fn pedido(&mut self, id: ClipId) {
    ...
    pub fn pedaco(&mut self, id: ClipId, index: u32, data: &[u8]) {
    ...
    pub fn fim(&mut self, id: ClipId) -> Option<ClipText> {
    ...
    /// O próximo pedaço a mandar, se há algum na fila.
    pub fn proximo_pedaco(&mut self) -> Option<ClipboardMessage> {
        self.fila.pop_front()
    }

    /// O par recusou a oferta: ela deixa de valer, e nada dela sai.
    pub fn recusaram(&mut self, id: ClipId) {
        self.oferta.take_if(|(oferecida, _)| *oferecida == id);
    }
}
