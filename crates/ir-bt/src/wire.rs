...
//! # O que é deste crate
//!
//! Só os números do rádio: a largura do prefixo e o teto do corpo. O modo, a espécie, o
//! desenquadrador e a contagem são os mesmos da rede, e moram no [`ir_crypto::enlace`] — a seta de
//! [02, §2](../../../docs/02-arquitetura.md) não deixa um transporte depender do outro, e a camada
//! em comum fica abaixo dos dois.

use ir_crypto::enlace;
/// O modo, a espécie e o embrulho de cada um: os mesmos da rede.
pub use ir_crypto::enlace::{
    Kind, Mode, corpo_de_handshake, desembrulhar, embrulhar, ler_handshake,
};
use ir_proto::limits;

// `BtError` só aparece na documentação e nos testes; o erro vem do `From` de `error`.
#[cfg(any(test, doc))]
...
pub fn enquadrar(corpo: &[u8]) -> Result<Vec<u8>> {
    Ok(enlace::enquadrar::<PREFIXO, MAX_CORPO>(corpo)?)
}
...
/// socket entregou, do tamanho que vier, e peça corpos até não haver mais nenhum inteiro. O
/// trabalho é o do desenquadrador do `ir-crypto`, o mesmo do TCP de arquivos; aqui se fixa o
/// prefixo `u16` e o teto do rádio.
#[derive(Debug, Default)]
pub struct Desenquadrador(enlace::Desenquadrador<PREFIXO, MAX_CORPO>);

impl Desenquadrador {
    /// Um desenquadrador vazio.
    #[must_use]
    pub const fn novo() -> Self {
        Self(enlace::Desenquadrador::novo())
    }
